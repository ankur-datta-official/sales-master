...
import { createCollectionTargetAction } from "@/modules/collection-targets/actions";
import {
...
export function CreateCollectionTargetForm({
  organizationId,
  assignableProfiles,
  parties,
}: Props) {
  ...
  const form = useForm<CreateCollectionTargetInput>({
    resolver: zodResolver(createCollectionTargetSchema),
    defaultValues: {
  ...
  function onSubmit(values: CreateCollectionTargetInput) {
    setError(null);
    startTransition(async () => {
      const result = await createCollectionTargetAction({
        ...values,
        organization_id: organizationId,
      });
      if (!result.ok) {
      ...
      router.push(`${ROUTES.collectionTargets}/${result.data!.collectionTargetId}`);
      router.refresh();
    });
  }
  ...
  return (
    <Card className="max-w-2xl">
      <CardHeader>
        <CardTitle>New collection target</CardTitle>
        <CardDescription>
          Assign collection goals within your hierarchy scope (amount only in v1).
        </CardDescription>
      </CardHeader>
