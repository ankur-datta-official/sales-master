...
import { reviewWorkReportAction } from "@/modules/work-reports/actions";
import {
...
export function ReviewWorkReportForm({ workReportId }: Props) {
  const router = useRouter();
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  const form = useForm<ReviewWorkReportInput>({
    resolver: zodResolver(reviewWorkReportSchema),
    defaultValues: {
      workReportId,
      status: "approved",
      review_note: "",
  ...
  function onSubmit(values: ReviewWorkReportInput) {
    startTransition(async () => {
      setError(null);
      const result = await reviewWorkReportAction({
        ...values,
        workReportId,
