...
import { updateSalesTargetAction } from "@/modules/sales-targets/actions";
import {
...
type Props = {
  salesTarget: SalesTargetWithRelations;
  assignableProfiles: ProfileOption[];
  parties: PartyOption[];
};

function qtyToInput(q: number | null): string | number {
  if (q === null || q === undefined) return "";
  return q;
}

export function EditSalesTargetForm({
  salesTarget,
  assignableProfiles,
  parties,
}: Props) {
  ...
  const partyRows = parties.some((p) => p.id === salesTarget.party_id)
    ? parties
    : salesTarget.party_id
      ? [
          { id: salesTarget.party_id, name: salesTarget.party_name ?? salesTarget.party_id },
          ...parties,
        ]
      : parties;

  const profileRows = assignableProfiles.some(
    (p) => p.id === salesTarget.assigned_to_user_id
  )
    ? assignableProfiles
    : [
        {
          id: salesTarget.assigned_to_user_id,
          full_name: salesTarget.assignee_name,
          email: salesTarget.assignee_email,
        },
        ...assignableProfiles,
      ];

  const form = useForm<UpdateSalesTargetInput>({
    resolver: zodResolver(updateSalesTargetSchema),
    defaultValues: {
      salesTargetId: salesTarget.id,
      assigned_to_user_id: salesTarget.assigned_to_user_id,
      party_id: salesTarget.party_id ?? "",
      ...
      target_qty: qtyToInput(salesTarget.target_qty),
  ...
  function onSubmit(values: UpdateSalesTargetInput) {
    ...
    startTransition(async () => {
      const result = await updateSalesTargetAction({
        ...values,
        salesTargetId: salesTarget.id,
      });
      if (!result.ok) {
  ...
  return (
    <Card className="max-w-2xl">
      <CardHeader>
        <CardTitle>Edit sales target</CardTitle>
