...
import {
  isCollectionTargetPeriodType,
  isCollectionTargetStatus,
...
export default async function CollectionTargetsPage({ searchParams }: PageProps) {
  const {
    dateFrom = "",
    dateTo = "",
    status = "",
  ...
  if (status && isCollectionTargetStatus(status)) {
    query = query.eq("status", status);
  }
  if (period && isCollectionTargetPeriodType(period)) {
  ...
  if (error) {
    return (
      <div className="space-y-2">
        <h1 className="text-2xl font-semibold tracking-tight">Collection targets</h1>
        <p className="text-destructive text-sm">Could not load collection targets: {error.message}</p>
      </div>
    );
  }

  const rows: CollectionTargetWithRelations[] = (data ?? []).map((row) =>
    mapCollectionTargetRow(row as Parameters<typeof mapCollectionTargetRow>[0])
  );
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Collection goals by period. Date range filters targets that overlap the range. Marketers
            see only targets assigned to them.
          </p>
        </div>
        {canManageCollectionTargets(role) && (
          <Link
            href={ROUTES.collectionTargetsNew}
              ...
              rows.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  ...
                  <td className="px-3 py-2">
                    <Link
                      href={`${ROUTES.collectionTargets}/${row.id}`}
